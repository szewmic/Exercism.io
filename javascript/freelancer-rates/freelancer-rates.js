...
export function monthRate(ratePerHour, discount) {
  let monthRate = dayRate(ratePerHour) * BILLABLE_DAYS_IN_MONTH;
  if (isDiscountValid(discount)) {
    monthRate = applyDiscount(monthRate, discount);
  };
  return monthRate;
}
...
export function daysInBudget(budget, ratePerHour, discount) {
  let dr = dayRate(ratePerHour);
  if (isDiscountValid(discount)) {
    dr = applyDiscount(dr, discount);
  };
...
function isDiscountValid(discount, minDiscount = MIN_DISCOUNT, maxDiscount = MAX_DISCOUNT) {
